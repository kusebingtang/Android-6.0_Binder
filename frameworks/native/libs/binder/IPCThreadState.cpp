                                  ...
                                  Parcel* reply, uint32_t flags)
{
    ...
    flags |= TF_ACCEPT_FDS;//允许回复中包含文件描述符
    ...
    if (err == NO_ERROR) {
        ...
        err = writeTransactionData(BC_TRANSACTION, flags, handle, code, data, NULL);  //整理数据，并把结果存入 mOut 中。(在 talkWithDriver方法中才会将命令真正发送给 Binder驱动)
    }
    ...
    if ((flags & TF_ONE_WAY) == 0) {    // oneway--不是异步，if命中
        #if 0
        if (code == 4) { // relayout
        ...
        err = waitForResponse(NULL, NULL);  // oneway，则不需要等待 reply 的场景
    }
...
status_t IPCThreadState::waitForResponse(Parcel *reply, status_t *acquireResult)
{
    ...
    while (1) {//循环等待结果
        if ((err=talkWithDriver()) < NO_ERROR) break;//TODO-->status_t IPCThreadState::talkWithDriver(bool doReceive)
        err = mIn.errorCheck();
        ...
        default:
            err = executeCommand(cmd);//处理 BR_NOOP命令，什么也没干
            if (err != NO_ERROR) goto finish;
            break;
        }
    }
...
status_t IPCThreadState::talkWithDriver(bool doReceive)
{
    ...
    const bool needRead = mIn.dataPosition() >= mIn.dataSize();//读的 buffer是否为空
    
    // We don't want to write anything if we are still reading
    // from data left in the input buffer and the caller
    // has requested to read the next data.
    const size_t outAvail = (!doReceive || needRead) ? mOut.dataSize() : 0;// 读的时 候不能写 mOut
    ...
    if (doReceive && needRead) {
        bwr.read_size = mIn.dataCapacity(); //TODO-->接收数据缓冲区信息的填充。如果以后收到数据，就直接填在mIn中了。
    ...
    do {
        ...
        //写入命令 BC_TRANSACTION
        if (ioctl(mProcess->mDriverFD, BINDER_WRITE_READ, &bwr) >= 0)//TODO-->通过ioctl不停的读写操作，跟Binder Driver进行通信
            err = NO_ERROR;
        else
            err = -errno;
#else
    ...
    int32_t handle, uint32_t code, const Parcel& data, status_t* statusBuffer)
{
    ...
    mOut.writeInt32(cmd);// cmd = BC_TRANSACTION ,  驱动找到 ServiceManager 后会像客户端返回一个 BR_TRANSACTION_COMPLETE 表示通信请求已被接受，然后 Client 进入等待
