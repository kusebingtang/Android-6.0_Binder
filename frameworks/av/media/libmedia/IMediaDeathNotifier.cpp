...
IMediaDeathNotifier::getMediaPlayerService()
{
    ...
    if (sMediaPlayerService == 0) {
        ...
        sp<IBinder> binder;
        do {
            //获取名为"media.player"的服务-->[BpServiceManager.getService]
            ...
            usleep(500000); // 0.5 s
        } while (true);//在请求获取名为”media.player”的服务过程中，采用不断循环获取的方法。由于MediaPlayerService服务可能还没向ServiceManager注册完成或者尚未启动完成等情况，故则binder返回为NULL，休眠0.5s后继续请求，直到获取服务为止。
        //将死亡通知连接到binder-->[]
